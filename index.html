...
for a in alerts:
  ...
  <div style="background:#111827;border:1px solid #1e2d42;border-radius:12px;padding:16px;margin:12px 0;border-left:3px solid #f0b429;">
    ...
    <div style="display:flex;justify-content:space-between;">
      <div>
        <div style="font-size:11px;color:#6b7a99;">INVESTI</div>
        <div style="font-family:monospace;color:#e8eaf0;">{a['montantInvesti']:.2f} €</div>
      </div>
      <div>
        <div style="font-size:11px;color:#6b7a99;">VALEUR ACTUELLE</div>
        <div style="font-family:monospace;color:#22c55e;font-size:18px;">{a['valeurActuelle']:.2f} €</div>
      </div>
      <div>
        <div style="font-size:11px;color:#6b7a99;">GAIN</div>
        <div style="font-family:monospace;color:#f0b429;font-size:18px;">+{gain:.2f} €</div>
      </div>
      <div>
...
for p in portfolio:
    ...
    prix_achat = float(p.get("prixAchat", 0))
    ...
    # Valeur actuelle = nb actions × prix actuel
    ...
    print(f"  Investi: {montant_investi:.2f} € | Valeur actuelle: {valeur_actuelle:.2f} € | Gain: {gain:+.2f} € ({pct:+.1f}%)")
    ...
    if valeur_actuelle >= objectif:
        ...
        print(f"  ⏳ Progression : {progress:.1f}% vers l'objectif x2")
